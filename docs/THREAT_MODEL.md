...
1. **Rust/WASM offline verifier — BUILT and integrated (2026-09-25).** The
   offline verifier now exists as a pure-Rust crate at `rust/verifier/`
   (native `anchor-verify` CLI + `wasm-bindgen` bindings; no network, no
   signing, no key generation). It verifies COSE_Sign1, ActionEnvelopes, v0
   `SignedEnvelope` checkpoint chains (hash links, gapless sequencing,
   signer rotation, emulated anchor proofs), Merkle inclusion, and holder
   proofs-of-possession, with strict deterministic CBOR and byte-exact
   canonical JSON. Cross-tested against fixtures generated by the real Python
   implementation. P0 security hardening applied: strict Ed25519
   (`verify_strict`, ed25519-dalek 3.x), 16 MiB input caps, decoder depth
   limits, checked arithmetic, locked supply chain (`cargo audit` /
   `cargo deny` in CI). The former blocker ("no Rust toolchain", blocker 11,
   LOG.md) is resolved. The Python verifier remains the reference
   implementation; the Rust verifier mirrors its verification halves.
